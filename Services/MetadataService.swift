...
final class EpisodeMetadataService {
    ...
    func fetchEpisodes(for media: AniListMedia, episodes: [SoraEpisode]) async -> [Int: EpisodeMetadata] {
        ...
        case .tmdb:
            ...
            let cacheKey = "episode-meta:tmdb-anilist:\(media.id):season:\(seasonNumber):count:\(desiredCount)"
            if let cached = cacheStore.readJSON(forKey: cacheKey, maxAge: 60 * 60 * 12),
               let decoded = try? JSONDecoder().decode([Int: EpisodeMetadata].self, from: cached) {
                return decoded
            }
            let fallback = await fetchFromAniList(mediaId: media.id)
            mapped = merge(primary: primary, fallback: fallback)
            if let data = try? JSONEncoder().encode(mapped) {
                cacheStore.writeJSON(data, forKey: cacheKey)
            }
        }
    ...
    private func fetchFromAniList(mediaId: Int) async -> [Int: EpisodeMetadata] {
        let episodes = (try? await aniListClient.streamingEpisodes(mediaId: mediaId)) ?? []
        var result: [Int: EpisodeMetadata] = [:]
        for ep in episodes {
            guard let number = ep.episodeNumber, number > 0 else { continue }
            let meta = EpisodeMetadata(
                number: number,
                title: ep.title,
                summary: nil,
    ...
    private func fetchFromTMDB(
        ...
        let seasonNumber = await resolveTMDBSeason(showId: showId, preferredSeason: preferredSeason, desiredCount: desiredCount)
    ...
    private func resolveTMDBSeason(showId: Int, preferredSeason: Int?, desiredCount: Int) async -> Int {
        ...
        do {
            ...
            let seasonNumbers = Set(seasons.compactMap { $0["season_number"] as? Int })
            if let preferredSeason, seasonNumbers.contains(preferredSeason) {
                return preferredSeason
            }
            guard desiredCount > 0 else { return 1 }
            var bestSeason = 1
            var bestDelta = Int.max
            for season in seasons {
                guard let number = season["season_number"] as? Int,
                      number > 0,
                      let count = season["episode_count"] as? Int else { continue }
                let delta = abs(count - desiredCount)
                if delta < bestDelta {
                    bestDelta = delta
                    bestSeason = number
                }
    ...
    private func merge(primary: [Int: EpisodeMetadata], fallback: [Int: EpisodeMetadata]) -> [Int: EpisodeMetadata] {
        guard !primary.isEmpty else { return fallback }
        var merged = primary
        for (number, fallbackMeta) in fallback {
            if let primaryMeta = merged[number] {
                let title = isGenericTitle(primaryMeta.title, number: number) ? fallbackMeta.title : primaryMeta.title
                let thumb = primaryMeta.thumbnailURL ?? fallbackMeta.thumbnailURL
                merged[number] = EpisodeMetadata(
                    number: number,
                    title: title,
                    summary: primaryMeta.summary ?? fallbackMeta.summary,
                    airDate: primaryMeta.airDate ?? fallbackMeta.airDate,
                    runtimeMinutes: primaryMeta.runtimeMinutes ?? fallbackMeta.runtimeMinutes,
                    thumbnailURL: thumb
                )
            } else {
                merged[number] = fallbackMeta
            }
        }
        return merged
    }

    private func isGenericTitle(_ title: String, number: Int) -> Bool {
        let trimmed = title.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if trimmed == "episode \(number)" { return true }
        if trimmed == "ep \(number)" { return true }
        if trimmed == "episode \(String(format: "%02d", number))" { return true }
        return false
    }
}
