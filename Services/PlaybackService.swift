...
struct PlaybackService {
    ...
    static func resolvePlayableURL(for sourceURL: URL, title: String?, episode: Int?) -> URL {
        if let title, let episode,
           ...
           let local = DownloadManager.shared.avPlayerURL(for: item) {
            AppLog.debug(.player, "playback resolve: matched download title=\(title) ep=\(episode) local=\(local.path) extension=\(local.pathExtension)")
            return local
        }
        if let item = DownloadManager.shared.items.first(where: { $0.url == sourceURL }),
           let local = DownloadManager.shared.avPlayerURL(for: item) {
            AppLog.debug(.player, "playback resolve: download item=\(item.id) local=\(local.path) extension=\(local.pathExtension)")
            return local
        }
