...
struct CinematicHeroView: View {
    ...
    let genres: [String]
    var height: CGFloat = UIConstants.heroHeight

    @EnvironmentObject private var appState: AppState
    @State private var assets: TMDBHeroAssets?
    ...
    var body: some View {
        ...
        .clipShape(RoundedRectangle(cornerRadius: 26, style: .continuous))
        .task(id: media.id) {
            assets = await appState.services.tmdbImageService.heroAssets(for: media)
        }
    }

    private var backgroundLayer: some View {
        Group {
            if let url = assets?.backdropURL ?? fallbackImageURL {
    ...
    private var genresText: String {
        let values = genres.isEmpty ? media.genres : genres
